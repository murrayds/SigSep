...
SS_COLUMN = 2;
CSV_OUTPUT_PATH = sprintf('%s\\quant_csvs\\', pwd);
PARAM_NAMES = {'Scenario', 'Algorithm', 'NFFT', 'Components'};
RESPONSE_NAMES = {'OPS', 'TPS', 'IPS', 'APS', 'TRAINTIME', 'SEPTIME'};
...
anovan_Tquick = cell(1, size(RESPONSE, 2));
...
for res = 1:size(RESPONSE, 2)
    ...
    for i = 2:17
           portion_variance = (tbl{i, SS_COLUMN} / total_variance) * 100;
           variance_column{i} = portion_variance;
    end
    ...
    summarized_filepath = sprintf('%ssummarized-anovan-csv-%s.csv', csv_dir, rname);
    writetable(T, table_filepath)
    writetable(summarized, summarized_filepath)
    anovan_p{res} = p;
    anovan_tbl{res} = tbl;
